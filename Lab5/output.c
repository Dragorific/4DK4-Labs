...
output_results(Simulation_Run_Ptr simulation_run)
{
    ...
    printf("Packet arrivals on Device 1 = %ld \n", data->arrival_count);
    printf("Packet arrivals on Device 2 = %ld \n", data->arrival_count2);
    ...
    printf("Transmitted packet count on Device 1  = %ld (Service Fraction = %.5f)\n\n",
     data->number_of_packets_processed, xmtted_fraction);

    printf("Arrival rate = %.3f packets/second \n", (double) data->arrival_rate23);
    printf("Transmitted packet count on Device 2  = %ld (Service Fraction = %.5f)\n",
